...
const { items, loading, saving, error, formError, fetchAll, save } = useAdminCrud<TipoNotificacion>('/tipo-notificaciones')

const showModal = ref(false)
const editing   = ref<TipoNotificacion | null>(null)
const form      = ref({ nombre: '', precio: '', color: '#6366f1', requiereCobro: false })

function openCreate() {
  editing.value = null
  form.value = { nombre: '', precio: '', color: '#6366f1', requiereCobro: false }
  showModal.value = true
}

function openEdit(item: TipoNotificacion) {
  editing.value = item
  form.value = {
    nombre: item.nombre,
    precio: item.precio != null ? String(item.precio) : '',
    color: item.color ?? '#6366f1',
    requiereCobro: Boolean(item.requiereCobro),
  }
  showModal.value = true
}
  ...
  const ok = await save(
    {
      ...
      requiereCobro: form.value.requiereCobro ? 1 : 0,
    },
    editing.value?.idTipoNotificacion,
  )
  if (ok) close()
}
...
<template>
  <div class="p-6 lg:p-8 max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-xl font-bold text-slate-800">Tipos de Notificación</h2>
      ...
      <table v-else class="w-full text-sm">
        <thead>
          <tr class="border-b border-slate-100 bg-slate-50 text-xs font-semibold text-slate-500 uppercase tracking-wide">
            ...
            <th class="text-right px-5 py-3.5">Precio</th>
            <th class="text-left px-5 py-3.5">Color</th>
            <th class="text-left px-5 py-3.5">Requiere Cobro</th>
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          <tr v-for="item in items" :key="item.idTipoNotificacion" class="hover:bg-slate-50 transition-colors">
            <td class="px-5 py-3.5 text-slate-400 font-mono text-xs">#{{ item.idTipoNotificacion }}</td>
            <td class="px-5 py-3.5 font-medium text-slate-800">{{ item.nombre }}</td>
            <td class="px-5 py-3.5 text-right text-slate-700">{{ formatPrecio(item.precio) }}</td>
            ...
            <td class="px-5 py-3.5">
              <span :class="['inline-flex px-2.5 py-1 rounded-full text-xs font-medium', item.requiereCobro ? 'bg-warning-50 text-warning-600' : 'bg-slate-100 text-slate-500']">
                {{ item.requiereCobro ? 'Sí' : 'No' }}
              </span>
            </td>
    ...
    <CrudModal :show="showModal" :title="editing ? 'Editar tipo de notificación' : 'Nuevo tipo de notificación'" :saving="saving" @close="close" @submit="submit">
      <div class="flex flex-col gap-4">
        ...
        <div>
          ...
          <input v-model="form.nombre" type="text" placeholder="Ej: Llamada de cobro" class="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition" />
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1.5">Precio <span class="text-slate-400 text-xs font-normal">(opcional)</span></label>
          <input v-model="form.precio" type="number" min="0" placeholder="0" class="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition" />
        </div>
        <div>
        ...
        <div class="flex items-center justify-between py-1">
          <div>
            <p class="text-sm font-medium text-slate-700">Requiere cobro</p>
            <p class="text-xs text-slate-400">Esta notificación genera un cargo al cliente</p>
          </div>
          <button
            type="button"
            @click="form.requiereCobro = !form.requiereCobro"
            :class="['relative w-11 h-6 rounded-full transition-colors', form.requiereCobro ? 'bg-primary-600' : 'bg-slate-300']"
          >
            <span :class="['absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform', form.requiereCobro && 'translate-x-5']" />
          </button>
        </div>
      </div>
    </CrudModal>
