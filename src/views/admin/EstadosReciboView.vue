...
const { items, loading, saving, error, formError, fetchAll, save } = useAdminCrud<EstadoRecibo>('/estado-recibos')

const showModal = ref(false)
const editing   = ref<EstadoRecibo | null>(null)
...
function openEdit(item: EstadoRecibo) {
  editing.value = item
...
async function submit() {
  const ok = await save({ nombre: form.value.nombre, color: form.value.color }, editing.value?.idEstadoRecibo)
  if (ok) close()
}
...
<template>
  <div class="p-6 lg:p-8 max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-xl font-bold text-slate-800">Estados de Recibo</h2>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
      ...
      <table v-else class="w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          <tr v-for="item in items" :key="item.idEstadoRecibo" class="hover:bg-slate-50 transition-colors">
            <td class="px-5 py-3.5 text-slate-400 font-mono text-xs">#{{ item.idEstadoRecibo }}</td>
    ...
    <CrudModal :show="showModal" :title="editing ? 'Editar estado' : 'Nuevo estado'" :saving="saving" @close="close" @submit="submit">
      <div class="flex flex-col gap-4">
        ...
        <div>
          ...
          <input v-model="form.nombre" type="text" placeholder="Ej: Pendiente" class="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition" />
        </div>
        <div>
